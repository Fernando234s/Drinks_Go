...
function criarCardPedido(pedido) {
  ...
  article.innerHTML = `
    ...
    <div class="order-summary">
      <p>Resumo do pedido e controle de status no mesmo card.</p>
      ${
        itens.length
          ? `<div class="order-items-list">${itens
              ...
              .join("")}</div>`
          : "<p class='muted-text'>Nenhum item informado.</p>"
      }
    </div>
  ...
  try {
    ...
    if (!pedidos.length) {
      listaPedidos.innerHTML = criarEmptyState(
        ...
        "Finalize um pedido no carrinho para acompanhar o fluxo aqui."
      );
      return;
    }
    ...
    listaPedidos.innerHTML = criarEmptyState(
      ...
      "Confira se a API local esta em execucao e tente novamente."
    );
  }
}
